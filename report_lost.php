<?php
// report_lost.php – SPATIAL INTELLIGENCE FORM (Aeon Campus Design System)
...
$message = '';
$error = '';
$item_name = $category = $description = $location = $report_date = $incident_time = '';
$latitude = $longitude = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf_token']) || !validateCsrfToken($_POST['csrf_token'])) {
        ...
        if (empty($item_name) || empty($category) || empty($description) || empty($report_date) || empty($incident_time)) {
            ...
            if (isset($_FILES['images']) && !empty($_FILES['images']['name'][0])) {
                ...
                for ($i = 0; $i < min($total_files, 5); $i++) {
                    if ($_FILES['images']['error'][$i] === UPLOAD_ERR_OK) {
                        ...
                        $filename = 'uploads/lost_' . time() . '_' . bin2hex(random_bytes(4)) . '_' . $i . '.' . $allowed[$mime];
            ...
            if (!$upload_failed) {
                // Insert with new Spatial Columns
                $stmt = $conn->prepare("INSERT INTO item_reports (user_id, report_type, item_name, category, description, location, latitude, longitude, search_radius, location_method, campus_location_id, report_date, incident_time, image_path, status) VALUES (?, 'lost', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'reported')");
                
                $stmt->bind_param("issssddisisss", $user_id, $item_name, $category, $description, $location, $latitude, $longitude, $search_radius, $location_method, $campus_location_id, $report_date, $incident_time, $primary_image);
                ...
                if ($stmt->execute()) {
                    ...
                    $message = '✅ Lost item reported securely with spatial data!';
                    
                    // AI Match integration
                    require_once 'ai_auto_match.php';
                    $ai_description = $description . " [Time Logged: " . $incident_time . "]";
                    $matches = aiAutoMatch($report_id, 'lost', $category, $location, $ai_description, $primary_image);
                    if ($matches > 0) $message .= " 🤖 AI found $matches match(es)! Check dashboard.";
...
<head>
    ...
    <title>CampusFind - Report Lost Item</title>
    ...
    <style>
        ...
        .btn-danger-glass { background: rgba(255, 68, 68, 0.15); border: 1px solid rgba(255, 68, 68, 0.3); color: #ffb4ab; transition: all 0.3s ease; cursor: pointer;}
        .btn-danger-glass:hover { background: rgba(255, 68, 68, 0.25); box-shadow: 0 0 20px rgba(255, 68, 68, 0.2); color: white; }
        ...
        .btn-outline-glass.active-rad { background: rgba(255, 68, 68, 0.2); border-color: #EF4444; color: #FCA5A5; }
        .input-glass { background: rgba(0, 0, 0, 0.2); border: 1px solid rgba(140, 144, 159, 0.3); transition: all 0.3s ease; }
        .input-glass:focus-within, .input-glass.active-dropdown { border-color: #ffb4ab !important; box-shadow: 0 0 10px rgba(255, 180, 171, 0.2) !important; }
        ...
        /* Updated Dropzone CSS */
        .dropzone { border: 2px dashed rgba(255, 180, 171, 0.4); background: rgba(0,0,0,0.2); transition: all 0.3s ease; cursor: pointer; min-height: 240px;}
        .dropzone:hover, .dropzone.dragover { border-color: #ffb4ab; background: rgba(255, 180, 171, 0.05); }
...
<body class="antialiased font-body-md selection:bg-error selection:text-white relative min-h-screen pb-20">
...
<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 pb-10 fade-in-up visible">
    <div class="mb-8 flex items-center gap-3">
        <div class="w-12 h-12 rounded-xl bg-error/20 border border-error/30 flex items-center justify-center">
            <span class="material-symbols-outlined text-error text-2xl">warning</span>
        </div>
        <div>
            <h1 class="text-3xl font-bold text-white tracking-tight">Report Lost Item</h1>
            <p class="text-on-surface-variant text-sm mt-1">Our AI engine will instantly scan for matches in the database.</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            <form id="reportForm" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="glass-panel p-6 sm:p-8 rounded-2xl">
                    <h3 class="text-xl text-white mb-6 flex items-center gap-2 border-b border-white/10 pb-4"><span class="material-symbols-outlined text-error">info</span> Basic Details</h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            ...
                            <div class="relative input-glass rounded-xl flex items-center px-4 py-3">
                                ...
                                <input name="item_name" class="w-full bg-transparent border-none text-white focus:ring-0 p-0 text-sm" placeholder="e.g. Black HP Laptop" type="text" value="<?php echo htmlspecialchars($item_name); ?>" required/>
                            </div>
                        </div>
                        ...
                        <div class="md:col-span-2 relative custom-dropdown" id="categoryDropdownWrapper">
                            ...
                            <div id="categoryMenu" class="absolute left-0 right-0 top-full mt-2 glass-panel rounded-xl opacity-0 invisible transition-all duration-200 z-[100] overflow-hidden">
                                <div class="max-h-64 overflow-y-auto custom-scrollbar py-2">
                                    <?php
                                    $cat_options = ['Electronics & Gadgets' => 'Laptops, Phones, Chargers', 'Cards, IDs & Documents' => 'Student Cards, IC', 'Bags & Apparel' => 'Backpacks, Jackets', 'Personal Belongings' => 'Wallets, Water Bottles', 'Sports & Equipment' => 'Rackets, Gym Gear', 'Other' => 'Miscellaneous'];
                                    foreach($cat_options as $mainCat => $subText) {
                                        echo "<div class='px-4 py-2 hover:bg-white/10 cursor-pointer transition-colors custom-option-cat text-sm text-white' data-value='$mainCat'><strong>$mainCat</strong><div class='text-[11px] text-white/50'>$subText</div></div>";
                                    }
                                    ?>
                                </div>
                            </div>
                        ...
                        <div>
                            <label class="block text-on-surface-variant mb-2 text-sm font-semibold">Date Lost <span class="text-error">*</span></label>
                            <div class="relative input-glass rounded-xl flex items-center px-4 py-3">
                        ...
                        <div class="relative custom-dropdown" id="timeDropdownWrapper">
                            ...
                            <div id="timeMenu" class="absolute left-0 right-0 top-full mt-2 glass-panel rounded-xl opacity-0 invisible transition-all duration-200 z-[100] overflow-hidden">
                                <div class="max-h-64 overflow-y-auto custom-scrollbar py-2">
                                    <?php
                                    $time_options = ['Morning (06:00 - 12:00)' => 'Early classes', 'Afternoon (12:00 - 18:00)' => 'Lunch, lectures', 'Evening (18:00 - 00:00)' => 'Dinner, events', 'Night (00:00 - 06:00)' => 'Late night', 'Uncertain' => 'Not sure exactly when'];
                                    foreach($time_options as $mainTime => $subText) {
                                        echo "<div class='px-4 py-2 hover:bg-white/10 cursor-pointer transition-colors custom-option-time text-sm text-white' data-value='$mainTime'><strong>$mainTime</strong><div class='text-[11px] text-white/50'>$subText</div></div>";
                                    }
                                    ?>
                                </div>
                            </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="relative input-glass rounded-xl px-4 py-3">
                                <textarea name="description" class="w-full bg-transparent border-none text-white focus:ring-0 p-0 text-sm min-h-[100px] resize-none" placeholder="Describe brand, color, unique marks. The AI uses this to match items." required><?php echo htmlspecialchars($description); ?></textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="glass-panel p-6 sm:p-8 rounded-2xl">
                    <h3 class="text-xl text-white mb-6 flex items-center gap-2 border-b border-white/10 pb-4"><span class="material-symbols-outlined text-error">add_a_photo</span> Visual Fingerprint</h3>
                    ...
                    <label for="imageInput" class="block w-full rounded-2xl dropzone p-8 text-center group transition-all relative overflow-hidden cursor-pointer mb-4 min-h-[200px] flex flex-col items-center justify-center">
                        ...
                        <div id="uploadPrompt" class="flex flex-col items-center justify-center transition-all duration-300 w-full h-full">
                            <div class="w-16 h-16 rounded-full bg-surface-container border border-white/10 flex items-center justify-center mx-auto mb-4 group-hover:bg-error/20 group-hover:border-error/50 transition-all duration-300">
                                <span class="material-symbols-outlined text-3xl text-error">add_photo_alternate</span>
                            </div>
                            <p class="text-white font-bold mb-1 text-lg">Click or drag photos here</p>
                        ...
                        <div id="imagePreviewContainer" class="hidden w-full h-full flex-col items-center justify-center">
                            ...
                            <div class="mt-6 text-sm font-bold text-error bg-error/10 px-4 py-2 rounded-full border border-error/20 hover:bg-error/20 transition-colors">
                ...
                <div class="glass-panel p-6 sm:p-8 rounded-2xl relative overflow-hidden border-t-4 border-error">
                    <div class="absolute top-0 right-0 w-32 h-32 bg-error/10 blur-3xl rounded-full pointer-events-none"></div>
                    <h3 class="text-xl text-white mb-2 flex items-center gap-2"><span class="material-symbols-outlined text-error">satellite_alt</span> Geospatial Location</h3>
                    ...
                    <div class="relative custom-dropdown mb-6" id="locationDropdownWrapper">
                        <div class="input-glass rounded-xl flex items-center px-4 py-3 cursor-pointer h-[50px] border-l-4 border-l-error" id="locationBtn">
                ...
                <div class="flex flex-col sm:flex-row gap-4 pt-4">
                    <button type="submit" class="btn-danger-glass text-white font-bold py-4 px-8 rounded-xl flex-1 flex items-center justify-center gap-2">
                        Submit Lost Report <span class="material-symbols-outlined text-[20px]">send</span>
                    </button>
                    <!-- SMART CANCEL BUTTON -->
        ...
        <div class="hidden lg:block space-y-6">
            <div class="glass-panel p-6 rounded-2xl sticky top-24 border-t-4 border-t-error">
                <div class="flex items-center gap-3 mb-4"><div class="w-10 h-10 rounded-full bg-error/20 flex items-center justify-center"><span class="material-symbols-outlined text-error">smart_toy</span></div><h3 class="text-lg font-bold text-white">AI Assistant</h3></div>
                <p class="text-sm text-on-surface-variant mb-6">Our neural engine scans every detail to help you recover your item faster.</p>
                <div class="space-y-4">
                    <div class="flex items-start gap-3"><span class="material-symbols-outlined text-error text-[20px]">satellite_alt</span><div><h4 class="text-sm font-bold text-white mb-0.5">Spatial Intelligence</h4><p class="text-xs text-white/60">The new map engine calculates exact radii to prevent false matches across campus.</p></div></div>
                    <div class="flex items-start gap-3"><span class="material-symbols-outlined text-error text-[20px]">image</span><div><h4 class="text-sm font-bold text-white mb-0.5">Clear Visuals</h4><p class="text-xs text-white/60">Uploading a reference photo allows the AI to compare shapes and colors directly.</p></div></div>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    map.on('load', () => {
        // Circle Radius Source (Red for Lost)
        map.addSource(radiusSourceId, { 'type': 'geojson', 'data': { 'type': 'FeatureCollection', 'features': [] } });
        map.addLayer({ 'id': 'radius-fill', 'type': 'fill', 'source': radiusSourceId, 'paint': { 'fill-color': '#EF4444', 'fill-opacity': 0.15, 'fill-outline-color': '#EF4444' } });

        // Building Polygon Source (Red for Lost)
        map.addSource(polySourceId, { 'type': 'geojson', 'data': { 'type': 'FeatureCollection', 'features': [] } });
        map.addLayer({ 'id': 'poly-fill', 'type': 'fill', 'source': polySourceId, 'paint': { 'fill-color': '#EF4444', 'fill-opacity': 0.3, 'fill-outline-color': '#ffb4ab' } });
    });
    ...
    // --- MAP HELPERS ---
    function dropRedPin(lat, lng) {
        if (currentMarker) currentMarker.remove();
        const el = document.createElement('div');
        el.className = 'w-8 h-8 rounded-full bg-error shadow-[0_0_15px_rgba(255,68,68,0.8)] border-2 border-white flex items-center justify-center text-white';
        el.innerHTML = '<span class="material-symbols-outlined text-[16px]">push_pin</span>';
        ...
        options.forEach(option => {
            option.addEventListener('click', (e) => {
                ...
                if (btnId === 'locationBtn') {
                    ...
                    options.forEach(opt => { 
                        opt.classList.remove('bg-error/20', 'text-error'); 
                    ...
                    option.classList.add('bg-error/20');
